...
import { getTeamById, updateTeam, getImageUrl } from "@/redux/Api";
...
export default function EditTeamPage() {
    ...
    const { team, loading: fetchLoading, error: fetchError } = useSelector((state: RootState) => state.getTeamById);
    const { loading: updateLoading, success: updateSuccess, error: updateError } = useSelector((state: RootState) => state.team);
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-50">
                <div className="flex flex-col items-center gap-4 text-slate-500">
                    ...
                    <p>{fetchError ? fetchError : "Loading team member..."}</p>
                </div>
            </div>
        );
    }
                                ...
                                <div className="p-8">
                                    ...
                                    <div className="mt-8 flex flex-col-reverse gap-4 sm:flex-row sm:justify-end bg-slate-50 p-4 -mx-8 -mb-8">
                                        <ResetButton onReset={() => {
                                            resetForm();
                                            setImageFile(null);
                                            setImagePreview(team?.image ? getImageUrl(team.image) : null);
                                        }} />
                                        <SubmitButton text={updateLoading ? "Updating..." : "Update Member"} type="submit" />
                                    </div>
                                </div>
                            </div>
                        </Form>
